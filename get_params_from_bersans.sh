...
if [ "$#" -lt 1 ]; then
	...
	echo "     get_params_from_bersans <list of files> -p <list of params>"
	echo "e.g. get_params_from_bersans *.004 -p SampleName Collimation SD Lambda Temperature TotalTime TotalSum Attenuator MaskFile"
	exit
fi
...
if [[ ${#filelist[@]} == 0 || ${#paramlist[@]} == 0 ]] ; then
	echo "At least one file and parameter mus tbe provided. Exit."
	exit
fi
...
do
	...
	do
		...
		val=`sed -e 's/\r$//' $file | sed -n "s/^.*$param=//p" | cut -d "<" -f1`
		val="$val"
